...
namespace RA
{
    ...
    public abstract class WorkGiver_WorkWithTools : WorkGiver
    {
        ...
        public bool ShouldKeepTool(Pawn pawn)
        {
            ...
            {
                if (toolComp.Allows("Hunting") && pawn.workSettings.WorkIsActive(WorkTypeDefOf.Hunting) && WorkGiver_Hunt.AvailableTargets(pawn).Any())
                {
                    return true;
                }
                if (toolComp.Allows("Construction") && pawn.workSettings.WorkIsActive(WorkTypeDefOf.Construction) && WorkGiver_ConstructFinishFrames.AvailableTargets(pawn).Any() ||
                    (pawn.workSettings.WorkIsActive(WorkTypeDefOf.Repair) && WorkGiver_Repair.AvailableTargets(pawn).Any()))
                {
                    return true;
                }
                if (toolComp.Allows("Mining") && pawn.workSettings.WorkIsActive(WorkTypeDefOf.Mining) && WorkGiver_Mine.AvailableTargets(pawn).Any())
                {
                    return true;
                }
                if (toolComp.Allows("Woodchopping") && pawn.workSettings.WorkIsActive(WorkTypeDefOf.PlantCutting) &&
                    WorkGiver_ChopWood.AvailableTargets(pawn).Any())
                {
                    return true;
                }
                if (toolComp.Allows("Digging") && pawn.workSettings.WorkIsActive(WorkTypeDefOf.Growing) &&
