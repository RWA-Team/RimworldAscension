...
namespace RA
{
    public static class RA_DefGenerator
    {
        ...
        {
            var defGenerators = ThingDefGenerator_Buildings.ImpliedBlueprintAndFrameDefs().Concat(ThingDefGenerator_Seeds.ImpliedSeedDefs()).Concat(ThingDefGenerator_Corpses.ImpliedCorpseDefs()).Concat(PlaceholdersDefGenerator.ImpliedPlaceholdersDefs().Concat(MinifiedDefGenerator.ImpliedMinifiedDefs()));
