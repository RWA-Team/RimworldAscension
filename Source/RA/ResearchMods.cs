...
namespace RA
{
    public static class ResearchMods
    {
        ...
        {
            foreach (var tableDef in DefDatabase<ThingDef>.AllDefs.Where(tableDef
                => !tableDef.recipes.NullOrEmpty()
                   && !tableDef.researchPrerequisites.NullOrEmpty()
                   && tableDef.recipes.Any(recipeDef => recipeDef.AvailableNow)))
            {
